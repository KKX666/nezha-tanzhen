...
最简单搭建方法 免申请SSL 免开启CDN 免反代 免宝塔 免域名 会占用80端口 请确保80端口是否占用
----------------------------------------------------------------------------------------------------------

**第一步：VPS连接SSH**

**第二步：去GitHub创建一个OAuth Apps和以上前部分同理（域名换成主机IP就好了）**

![image](https://user-images.githubusercontent.com/94978556/149937982-dd413b64-cc2d-4d82-8451-6ddafa76b6a7.png)


**第三步：选择1安装面板和以上同理**

**第四步：访问`http://`主机IP＋默认端口:8008(如果改了 请填你的）**
...
（添加被监控机到这一步时 请输入一个解析到面板所在IP的域名（不可套CDN）:填写你的主机IP）

（如果登陆后访问出错请手动在链接后加:8008)

（如果不想露IP地址  可以解析一个域名  无需套小云朵）
